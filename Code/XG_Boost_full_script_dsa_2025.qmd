...
**XGBoost** (Extreme Gradient Boosting) is a powerful machine learning algorithm based on **gradient boosting**. It’s widely used for **tabular/structured data**, especially in competitive data science settings.

XGBoost builds decision trees **sequentially**, where each new tree corrects the errors of the previous ones. Unlike Random Forest (which builds trees independently), XGBoost focuses on minimizing a loss function using **gradient descent**.
...
- Engineer **lag features** to capture trends and seasonality.
...
We can search for that in this link: https://www.tmwr.org/pre-proc-table   #Don't need for numeric data
...
- [**Tidy Modeling Book Club**](https://r4ds.github.io/bookclub-tmwr/)  
  This community-driven book club breaks down the _Tidy Modeling with R_ book into digestible sections, complete with meeting notes and examples—great for collaborative learning.
