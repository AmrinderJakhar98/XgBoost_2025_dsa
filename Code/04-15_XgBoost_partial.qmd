...
Our learning objectives are to: - Revise previous models\
- Understand the XGBoost algorithm\
- Use the ML framework to: - Pre-process data\
...
XGBoost is built upon several core concepts: - **Supervised Machine Learning** - **Decision Trees** - **Ensemble Learning** - **Gradient Boosting**
...
XGBoost also supports: - **Parallel tree construction** - **Regularization** (to prevent overfitting) - **Handling of missing values** - **Custom loss functions** - **Efficient memory usage and computation**
...
![Boosted Trees](https://bradleyboehmke.github.io/HOML/images/boosted-trees-process.png)
...
![Boosting Diagram](https://miro.medium.com/v2/resize:fit:1400/format:webp/0*fdDu8RbNLoUzrrlF.jpeg)
...
-   Generalizes better than individual learners

## Core Steps in Gradient Boosting

1.  **Optimize Loss Function**
    -   Use a differentiable loss function.
    -   Minimize it using gradient descent.
2.  **Train Weak Learners (Trees)**
    -   Fit trees to residuals/errors from previous steps.
3.  **Add Trees Sequentially**
    -   Each tree incrementally improves the prediction.
    -   Stop when loss reduction plateaus or a tree limit is reached.
...
![Tree Depth Effect](https://www.baeldung.com/wp-content/uploads/sites/4/2020/05/Screenshot-2020-05-12-at-07.03.52-1024x695.png)\
![Early Stopping Trees](https://bradleyboehmke.github.io/HOML/07-decision-trees_files/figure-html/dt-early-stopping-1.png)
...
For time series tasks: - **Avoid random splits**—preserve temporal order. - **Train on the past**, **test on the future**. - Use **rolling-window cross-validation**. - Engineer **lag features** to capture trends and seasonality.

💡 Tip: XGBoost doesn’t inherently understand time, so you must encode it via lag features or rolling averages.
...
- **`trees`**: The number of boosting rounds (i.e., how many trees will be added sequentially). - **`tree_depth`**: Controls how deep each individual tree can grow. Deeper trees can capture more complex interactions but also increase the risk of overfitting. - **`min_n`**: Minimum number of observations required in a node for it to be split. Acts as a regularization tool to prevent overly specific splits. - **`learn_rate`**: Also known as `eta`, it controls how much each additional tree contributes to the overall model. Smaller values make the model more stable but require more trees.
...
We select the best models using three strategies (best, within 1 SE, within 2% loss) all thre strategies which we learn in class:
